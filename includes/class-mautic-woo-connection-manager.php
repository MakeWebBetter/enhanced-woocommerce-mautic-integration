...
class MauticWooConnectionMananager {
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( 200 === $status_code ) {
				...
				if ( ! empty( $api_body->refresh_token ) && ! empty( $api_body->access_token ) && ! empty( $api_body->expires_in ) ) {
					...
					$message = __( 'Fetching and refreshing access token', 'mautic-woo' );
					...
					$message = __( 'Something went wrong!. Please try again.', 'mautic-woo' );
				...
				$message = __( 'Something went wrong.', 'mautic-woo' );
				update_option( 'mautic_woo_api_validation_error_message', $message );
				update_option( 'mautic_woo_valid_client_ids_stored', false );
			}
		}
	...
	public function create_property( $prop_details ) {
		...
		$response = array(
			'code'    => 400,
			'message' => __( 'Failed in creating field', 'mautic-woo' ),
		);
		...
		if ( ! empty( $code ) && ! empty( $message ) ) {
			...
			$message = __( 'Creating Custom Fields', 'mautic-woo' );
	...
	public function create_or_update_contacts( $details ) {
		...
		$response = array(
			'code'    => 400,
			'message' => __( 'Failed to create or update contact.', 'mautic-woo' ),
		);
		...
		if ( ! empty( $code ) && ! empty( $message ) ) {
					$response = array(
						...
						$message = __( 'Updating or Creating users data', 'mautic-woo' );
