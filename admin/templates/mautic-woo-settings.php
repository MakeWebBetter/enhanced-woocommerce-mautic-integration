...
<div class="mauwoo-settings-header mauwoo-common-header">
	<h2><?php esc_html_e( 'General Settings', 'mautic-woo' ); ?></h2>
</div>
<div class="mauwoo-settings-container">
	<div class="mauwoo-general-settings">
		...
		if ( isset( $_POST['mautic_woo_save_gensttings'] ) && check_admin_referer( 'mautic-woo-settings' ) ) {
			...
			$message = esc_html__( 'Settings saved', 'mautic-woo' );
			Mautic_Woo::mautic_woo_notice( $message, 'success' );
		}
		?>
		<form action="" method="post">
			<div class="mauwoo-order-status">
				<label for="mauwoo-selected-order-status"><?php esc_html_e( 'Sync orders with status', 'mautic-woo' ); ?></label>
				<?php
			...
			<div class="mauwoo-order-status">
				<label for="mauwoo-disabled-custom-fields"><?php esc_html_e( 'Disable Custom fields', 'mautic-woo' ); ?></label>
				<?php
				$desc = esc_html__( 'The data sync will be stop for selected fields', 'mautic-woo' );
				echo wp_kses_post( wc_help_tip( $desc ) );/* phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped */
			...
			<div class="mauwoo-order-status">
				<label for="mauwoo-disabled-custom-fields"><?php esc_html_e( 'Custom Contact Tags', 'mautic-woo' ); ?></label>
				<?php
				$desc = esc_html__( 'Enter text to be added as tags to each contact. For multiple tags separate them with comma, i.e. tag1 , tag2', 'mautic-woo' );
				echo wp_kses_post( wc_help_tip( $desc ) ); /* phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped */
			...
			<div class="mauwoo-order-status">
				<label for="mautic_woo_sync_method"><?php esc_html_e( 'Preferred sync method', 'mautic-woo' ); ?></label>
				<?php
					$mautic_woo_sync_method = get_option( 'mautic_woo_sync_method', 'instant' );

					$attribute_description = esc_html__( 'Select your preferred method to sync user activity on Mautic.', 'mautic-woo' );
				...
				<select class="mauwoo-order-status" name="mautic_woo_sync_method">
					...
					if ( 'cron' === $mautic_woo_sync_method ) {
						?>
							<option selected value="cron"><?php esc_html_e( 'In every 5 minutes', 'mautic-woo' ); ?></option>
							<option value="instant"><?php esc_html_e( 'Instant Sync', 'mautic-woo' ); ?></option>
							<?php
					} else {
						?>
							<option value="cron"><?php esc_html_e( 'In every 5 minutes', 'mautic-woo' ); ?></option>
							<option selected value="instant"><?php esc_html_e( 'Instant Sync', 'mautic-woo' ); ?></option>
							<?php
					}
					?>
				</select>
			...
			<p class="submit">
				<input type="submit" class="mauwoo-button" name="mautic_woo_save_gensttings" value="<?php esc_html_e( 'Save settings', 'mautic-woo' ); ?>">
			</p>
